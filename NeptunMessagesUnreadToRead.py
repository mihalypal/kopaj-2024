...
def get_unread_messages(access_token: str, cookie: str) -> List[str]:
    ...
    unread_messages = [message for message in response.json()['data']['receivedMessages'] if
                       message['unreadedPostCount'] != 0]
...
def mark_messages_as_read(access_token: str, message_ids: List[str], cookie) -> None:
    ...
    for message_id in message_ids:
        ...
        post_ids_number -= 1
        time.sleep(
            0.2)  # You can change this number if you get connection error or something like that (the value is in seconds)
